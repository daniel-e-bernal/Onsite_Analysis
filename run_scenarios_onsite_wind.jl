...
@everywhere begin
    ...
    function run_site(i::Int)
        ...
        sleep(10)
        max_cap_num_of_turbines = data[!, :turbine_0_num_turbines][i]
        ...
        write(joinpath("C:/Users/dbernal/Documents/GitHub/Onsite_Analysis/results/Wind/inputs_REopt/", "$(file_name)_inputs_REopt.json"), JSON.json(inputs1))
        ...
        new_wind_data = select_wind_turbines(site_row, annual_load)
        ...
        for i in eachindex(wind_load_minus_prod_kwh_series)
            if wind_load_minus_prod_kwh_series[i] <= 0
                append!(wind_export_kwh_series, abs(wind_load_minus_prod_kwh_series[i]))
            else
                append!(wind_export_kwh_series, 0)
            end
        end
        for i in eachindex(wind_load_minus_prod_kwh_series)
            if wind_load_minus_prod_kwh_series[i] <= 0
                append!(grid_supplied_kwh, 0)
                append!(wind_serving_load_total_series, load_vector[i])
            else
                append!(grid_supplied_kwh, abs(wind_load_minus_prod_kwh_series[i]))
                append!(wind_serving_load_total_series, wind_prod_kwh_series[i])
            end
        end 
        ...
        df = DataFrame(
            ...
            input_wind_size_class = (new_wind_class),#[input_data_dic["PV"][1]["location"]],
            input_wind_num_of_turbines = (new_wind_data["Number of Turbines"]), 
            input_wind_total_total_cap = (new_wind_data["Total Capacity (kW)"]),
            #total metrics 
