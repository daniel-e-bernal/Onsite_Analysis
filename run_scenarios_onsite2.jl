...
@everywhere begin
    ...
    cols = [:MatchID, :naicsCode, :place_name, :latitude, :longitude, :parcel_latitude, :parcel_longitude, :state, :airport_5km_int, :urban_areas_int, :DOD_airspace_int, :critical_habs_1_int, :critical_habs_2_int, :wind_exclusion, :cejst_dac_int, :FLD_PFS, :WFR_PFS, :rooftop_area_m2, :solarPV_ground_area, :wind_ground_area, :under_1_acre]
    ...
    # File imports
    pv_roof_prod_factors = "C:/GitRepos/Onsite_Energy_temporary/PVWatts_/pvwatts_roof_csvs/pvwatts_roof_csvs/"
    pv_ground_prod_factors = "C:/GitRepos/Onsite_Energy_temporary/PVWatts_/pvwatts_ground_csvs/pvwatts_ground_csvs/"
    ...
    function run_site(i::Int)
        ...
        for name in [1, 2] #["ground_mount", "roof_fixed"]
            if input_data_site["PV"][name]["name"] == "ground_mount"
                ...
                #prod_factor = input_data_site["PV"][name]["production_factor_series"]
        ...
        write(joinpath("C:/GitRepos/Onsite_Energy_temporary/results/PV/analysis_runs/", "$(file_name)_analysis_runs.json"), JSON.json(analysis_runs))
        write(joinpath("C:/GitRepos/Onsite_Energy_temporary/results/PV/emissions/", "$(file_name)_emissions.json"), JSON.json(emissions))
        write(joinpath("D:/Onsite_energy_temporary/results/PV/inputs_REopt/", "$(file_name)_inputs_REopt.json"), JSON.json(inputs2))
        write(joinpath("C:/GitRepos/Onsite_Energy_temporary/results/PV/inputs_site/", "$(file_name)_inputs_data_site.json"), JSON.json(input_data_site))
        #sleep(1.0)

        # Set up extractable json file with all inputs to put onto DataFrame
        inputs_all = JSON.parsefile(joinpath("D:/Onsite_energy_temporary/results/PV/inputs_REopt/", "$(file_name)_inputs_REopt.json"))
        input_data_dic = JSON.parsefile(joinpath("C:/GitRepos/Onsite_Energy_temporary/results/PV/inputs_site/", "$(file_name)_inputs_data_site.json"))
        ...
        df = DataFrame(
            ...
            PV_max_possible_size_kw_ground = [round(inputs_all["max_sizes"]["ground_mount"], digits=4)],
            #roof PV inputs
            ...
            PV_max_possible_size_kw_roof = [round(inputs_all["max_sizes"]["roof_fixed"], digits=4)],
            #total metrics 
...
#rerun_data = CSV.read("C:/GitRepos/Onsite_Energy_temporary/rerun_solar.csv", DataFrame)
...
#evaluated_2 = readdir("C:/GitRepos/Onsite_Energy_temporary/results/PV/results_backup/")
@info size(scenarios)
#!(fname in evaluated) &&  
