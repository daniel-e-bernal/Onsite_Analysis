...
"""
This script is allowing the system to overproduce by adding 1 additional turbine below the turbine quantity to suffice <100% annual electric load capacity.
The condition above is only IF the site can host >1 turbine. If it can not host >1 turbine, then we leave it as 1 turbine.
"""
#using Distributed 
...
@everywhere begin
    ...
    new_cols = [:MatchID, :naicsCode, :place_name, :parcel_latitude, :parcel_longitude, :state, :airport_5km_int, :urban_areas_int, :DOD_airspace_int, :critical_habs_1_int, 
    ...
    function run_site(i::Int)
        ...
        if ismissing(turbine_step)
            ...
            if wind_max_size_based_on_load >= max_kw_based_on_space 
                ...
                #println("The new wind_size is ", wind_size)
                input_data_site["Wind"]["size_class"] = new_wind_class
                input_data_site["Wind"]["max_kw"] = new_max_wind_capacity
                #println("The new wind max size capacity is ", input_data_site["Wind"]["max_kw"])
            ...
            #println("Second check for new wind size is ", wind_size)
...
evaluated = readdir("C:/GitRepos/Onsite_Energy_temporary/results/Wind/results_turbine_0/")
